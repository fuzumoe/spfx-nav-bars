...
import { BaseComponent, autobind, buttonProperties, anchorProperties, css, divProperties, getId, getNativeProps } from '../../Utilities';
import { FocusZone, FocusZoneDirection } from '../../FocusZone';
import { ContextualMenu, hasSubmenuItems } from '../../ContextualMenu';
...
var CommandBar = /** @class */ (function (_super) {
    ...
    CommandBar.prototype.render = function () {
        ...
        if (isSearchBoxVisible) {
            searchBox = (React.createElement("div", { className: css('ms-CommandBarSearch', styles.search), ref: 'searchSurface' },
                ...
                React.createElement("div", { className: css('ms-CommandBarSearch-iconWrapper ms-CommandBarSearch-iconSearchWrapper', styles.searchIconWrapper, styles.searchIconSearchWrapper) }, Icon({ iconName: 'Search' })),
        ...
        return (React.createElement("div", { className: css('ms-CommandBar', styles.root, className), ref: 'commandBarRegion' },
            searchBox,
            React.createElement(FocusZone, { ref: 'focusZone', className: styles.container, direction: FocusZoneDirection.horizontal, role: 'menubar' },
                React.createElement("div", { className: css('ms-CommandBar-primaryCommands', styles.primaryCommands), ref: 'commandSurface' }, renderedItems.map(function (item, index) { return (_this._renderItemInCommandBar(item, index, expandedMenuItemKey)); }).concat((renderedOverflowItems && renderedOverflowItems.length) ? [
                    React.createElement("div", { className: css('ms-CommandBarItem', styles.item), key: OVERFLOW_KEY, ref: OVERFLOW_KEY },
                ...
                React.createElement("div", { className: css('ms-CommandBar-sideCommands', styles.sideCommands), ref: 'farCommandSurface' }, renderedFarItems.map(function (item, index) { return (_this._renderItemInCommandBar(item, index, expandedMenuItemKey, true)); }))),
            (contextualMenuProps) ?
    ...
    CommandBar.prototype._renderItemInCommandBar = function (item, index, expandedMenuItemKey, isFarItem) {
        ...
        var isLink = item.onClick || hasSubmenuItems(item);
        var className = css(isLink ? ('ms-CommandBarItem-link ' + styles.itemLink) : ('ms-CommandBarItem-text ' + styles.itemText), !item.name && ('ms-CommandBarItem--noName ' + styles.itemLinkIsNoName), (expandedMenuItemKey === item.key) && ('is-expanded ' + styles.itemLinkIsExpanded));
        ...
        var command;
        if (isLink) {
            command = (React.createElement("button", tslib_1.__assign({}, getNativeProps(item, buttonProperties), { id: this._id + item.key, className: className, onClick: this._onItemClick(item), "data-command-key": itemKey, "aria-haspopup": hasSubmenuItems(item), "aria-expanded": hasSubmenuItems(item) ? expandedMenuItemKey === item.key : undefined, role: 'menuitem', "aria-label": ariaLabel }),
                ...
                hasSubmenuItems(item) ? (React.createElement(Icon, { className: css('ms-CommandBarItem-chevronDown', styles.itemChevronDown), iconName: 'ChevronDown' })) : (null)));
        }
        else if (item.href) {
            command = (React.createElement("a", tslib_1.__assign({}, getNativeProps(item, anchorProperties), { id: this._id + item.key, className: className, href: item.href, "data-command-key": itemKey, "aria-haspopup": hasSubmenuItems(item), role: 'menuitem', "aria-label": ariaLabel }),
                (hasIcon) ? this._renderIcon(item) : (null),
                isNameVisible && (React.createElement("span", { className: css('ms-CommandBarItem-commandText', styles.itemCommandText) }, item.name))));
        }
        else {
            command = (React.createElement("div", tslib_1.__assign({}, getNativeProps(item, divProperties), { id: this._id + item.key, className: className, "data-command-key": itemKey, "aria-haspopup": hasSubmenuItems(item), "aria-label": ariaLabel }),
                (hasIcon) ? this._renderIcon(item) : (null),
    ...
    CommandBar.prototype._onItemClick = function (item) {
        var _this = this;
        return function (ev) {
            if (item.key === _this.state.expandedMenuItemKey || !hasSubmenuItems(item)) {
